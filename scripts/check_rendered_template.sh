...
if [[ "$package_name" != "testkitx" && -e "$repo_dir/pkg/testkitx" ]]; then
  echo "ERROR: stale pkg/testkitx directory still exists" >&2
  exit 1
fi
...
scan_fixed "github.com/ZoneCNH/testkitx" "module path"

if [[ "$module_name" != "testkitx" ]]; then
  scan_fixed "testkitx" "module name"
fi

if [[ "$package_name" != "testkitx" ]]; then
  scan_regex '\btestkitx\b' "package name"
fi
