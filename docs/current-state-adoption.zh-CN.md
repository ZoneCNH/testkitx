...
| Manifest fixture | `pkg/testkitx/manifesttest` | 构造、校验、写入 release manifest fixture 和 checksum sidecar |
...
- `manifesttest.Manifest`：记录 module、commit、gate 状态和 evidence 路径；`manifesttest` 写入 `.sha256` sidecar 并可检测 manifest 漂移。
...
7. 使用 `DOWNSTREAM_REPO=/path/to/downstream GOWORK=off make downstream-test-only-adoption-check` 或 `DOWNSTREAM_ADOPTION_EVIDENCE=/path/to/evidence.json GOWORK=off make downstream-test-only-adoption-check` 记录 test-only 采用证明。
8. 将 helper 产出的 Evidence 或 gate 输出作为 CI artifact 保存。
9. 在 PR 或发布说明中记录采用范围、通过的 gate 和任何未覆盖缺口。
...
- 至少一个真实下游仓库的采用 PR、commit 或测试输出证明；本仓库提供 `downstream-test-only-adoption-check` 入口，但仍需要真实下游路径或 Evidence 文件。
