# testkitx 完整可执行 Goal Prompt v1.0

> 文件用途：把本文件完整交给 Agent Teams / Codex / 自动化执行器，用于创建 `testkitx` 独立基础库模板仓库。
> 适用项目：x.go 基础库体系。
...
你是负责 x.go 基础设施资产化的工程 Agent。你的任务不是写一份说明文档，而是实际创建一个可复用、可测试、可发布、可治理的独立基础库模板仓库：`testkitx`。
...
```text
GOAL-20260601-001
建立 testkitx 独立基础库模板仓库，为 x.go 及其周边基础设施库提供统一模块脚手架、目录规范、公共文档、CI/Harness Gate、release evidence、复盘和自我改进机制，使后续 foundationx/postgresx/kafkax/redisx/taosx 等基础库可以低成本、标准化、可验证地生成。
```
...
```text
module path 默认：github.com/ZoneCNH/testkitx
Go 版本默认：1.23
...
echo "no external integration dependency for testkitx"
...
testkitx 是一个独立 Go 基础库模板模块。

## 目标

testkitx 是一个独立 Go 基础库模板模块，为创建独立基础库提供标准骨架。
...
将 testkitx 构建为 x.go 基础库的标准独立基础库模板。
```
...
```text
pkg/testkitx/config_test.go
pkg/testkitx/client_test.go
pkg/testkitx/health_test.go
...
```text
TASK-BT-004 实现 pkg/testkitx/config.go
TASK-BT-005 实现 pkg/testkitx/client.go
TASK-BT-006 实现 pkg/testkitx/options.go
TASK-BT-007 实现 pkg/testkitx/health.go
TASK-BT-008 实现 pkg/testkitx/errors.go
TASK-BT-009 实现 pkg/testkitx/metrics.go
TASK-BT-010 实现 pkg/testkitx/version.go/doc.go
```
...
## 新 Issue 候选
- ISSUE-FOUNDATIONX-001 从 testkitx 生成 foundationx。
- ISSUE-POSTGRESX-001 从 testkitx 生成 postgresx。
```
...
已创建：
- testkitx/go.mod
- testkitx/pkg/testkitx/*
- testkitx/scripts/*
- testkitx/.github/workflows/*
- testkitx/docs/*
- testkitx/contracts/*
- testkitx/.agent/*
- testkitx/release/manifest/latest.json
...
下一步：
- 从 testkitx 生成 foundationx。
- 从 testkitx 生成 postgresx。
```
...
现在开始执行，不要停留在解释层。请实际创建 `testkitx` 的目录、文件、脚本、CI、文档、contracts、`.agent`、release manifest，并运行验证命令。
