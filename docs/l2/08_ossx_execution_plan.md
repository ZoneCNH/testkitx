# ossx 执行方案：ObjectStore / Multipart / Presign
...
```text
bucket not found→not_found/config
object not found→not_found
access denied→permission_denied
invalid key→validation
checksum mismatch→integrity/serialization
upload timeout→timeout
presign expired→permission_denied/expired
```
...
```text
L2-T2 只开 objectstore。
L2-T3 增加 large object/pagination/chaos/benchmark/adoption。
L2-T4 打开 multipart/presign。
```
