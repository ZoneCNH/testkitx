...
本仓库是 Go 1.23 基础库模板，模块路径为 `github.com/ZoneCNH/testkitx`。公共 API 位于 `pkg/testkitx`；仅供内部使用的实现放在 `internal/`，当前包括 `sanitize`、`validation` 和 `runtime`。可复用测试工具在 `testkit/`，示例程序在 `examples/basic`、`examples/config` 和 `examples/health`。contracts 文件位于 `contracts/`，项目说明在 `docs/`，自动化脚本在 `scripts/`，发布 Evidence 生成到 `release/manifest/`。
...
- `make lint`：执行 `golangci-lint run ./...`；缺少 `golangci-lint` 时必须失败。启用的 linter 包括 govet、ineffassign、staticcheck、errcheck、gocritic、misspell、unconvert。
...
使用标准 Go 风格：交给 `gofmt` 处理缩进，包名保持简短，导出标识符要清晰表达用途。模板占位符如 `testkitx`、`github.com/ZoneCNH/testkitx`、`testkitx` 必须在代码和文档中保持一致。公共库能力放入 `pkg/testkitx`，私有辅助逻辑放入 `internal/`。不要新增隐藏全局客户端，不要隐式读取生产密钥，不要引入业务领域模型。
